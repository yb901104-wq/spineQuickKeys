...
后续真实资源制作与替换范围见 `resource-overview.md`。实际实现时以 `KeyMacro/Forms` 现有代码为功能真相源，概览图只决定默认尺寸、布局方向和美术风格。
...
- 主工具栏保持所有现有入口。
- 入口名称和数量按真实代码保留，例如开启虚拟按键、关闭虚拟按键、管理虚拟按键是三个独立按钮。
- 宏序列表格强化扫描效率。
- 状态栏保留配置、VK、热键等运行状态。
...
- 顶部集中显示序列名称、触发快捷键和关联虚拟按键，功能入口按真实代码保留为“键盘录入 / 虚拟按键 / 清除”。
- 步骤列表保持表格主导，列名按真实代码保留：类型、按键/文本、延迟、触发方式、按压时长、操作。
- 步骤工具栏按真实代码保留为“添加步骤 / 删除步骤 / 录制按键 / 上移 / 下移”。
- 确认/取消按钮统一放在右下角。
...
- 文件路径、载入、录制、搜索集中在顶部。
- 表格按真实代码保留为快捷键名称、快捷键、功能说明三列，分组文字并入名称内容展示。
